...
const { EvaluationPeriod, PeriodIndicator, Indicator, IndicatorCriterion } = require('../models');
...
router.put('/:id/indicators', authenticate, authorize('admin'), async (req, res) => {
    const { indicators } = req.body; // [{ indicator_id, weight, sort_order, is_active }]
    ...
    await Promise.all(indicators.map(({ indicator_id, weight, sort_order, is_active }) =>
        ...
        }, { conflictFields: ['period_id', 'indicator_id'] })
    ));
