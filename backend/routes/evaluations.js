...
const {
    ...
    EvaluationPeriod, PeriodIndicator, Department, User,
} = require('../models');
...
function computeRatioScore(raw_values) {
...
router.get('/matrix', authenticate, authorize('admin', 'qc_head'), async (req, res) => {
...
router.post('/', authenticate, authorize('admin', 'qc_head'), async (req, res) => {
    const { period_id, department_id, criterion_id, submission_id, score, raw_values, reviewer_notes, evaluation_method } = req.body;
    ...
    const [evaluation, created] = await Evaluation.findOrCreate({
        ...
        defaults: {
            submission_id, score: finalScore, raw_values: raw_values || {}, reviewer_notes,
...
router.get('/scores/indicators', authenticate, async (req, res) => {
    const { period_id, department_id } = req.query;
...
router.get('/scores/departments', authenticate, async (req, res) => {
...
router.get('/scores/colleges', authenticate, async (req, res) => {
    const { period_id } = req.query;
    if (!period_id) return res.status(400).json({ error: 'period_id is required' });
