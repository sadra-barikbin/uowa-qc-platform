...
async function assertDepartmentAccess(req, department_id) {
...
router.get('/', authenticate, async (req, res) => {
    const { period_id, department_id, criterion_id } = req.query;
    const where = {};
...
router.post('/', authenticate, authorize('admin', 'qc_head', 'dept_rep'), async (req, res) => {
    ...
    const [submission, created] = await Submission.findOrCreate({
        ...
        defaults: { notes, status: 'submitted', submitted_by: req.user.id, submitted_at: new Date() },
    });
...
router.post('/:id/documents', authenticate, authorize('admin', 'qc_head', 'dept_rep'), upload.array('files', 10), async (req, res) => {
    ...
    const documents = await SubmissionDocument.bulkCreate(req.files.map(f => ({
        submission_id: submission.id, file_name: f.originalname, storage_provider: 'local',
...
router.get('/documents/:docId/download', authenticate, async (req, res) => {
...
router.delete('/documents/:docId', authenticate, authorize('admin', 'qc_head', 'dept_rep'), async (req, res) => {
    const doc = await SubmissionDocument.findByPk(req.params.docId, { include: [{ model: Submission, as: 'submission' }] });
    if (!doc) return res.status(404).json({ error: 'Document not found' });
    if (!(await assertDepartmentAccess(req, doc.submission.department_id))) return res.status(403).json({ error: 'Not assigned to this department' });
